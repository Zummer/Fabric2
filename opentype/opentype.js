...
(function () {
    ...
    Path.prototype.getLength = function () {
        var i, cmd, dstroke,dstrokeArray, cmdstroke;
        dstrokeArray = [];

        for (i = 0; i < this.commands.length; i += 1) {
            cmd = this.commands[i];
            cmdstroke = cmd.type + " " + cmd.x + " " + cmd.y;
            dstrokeArray.push(cmdstroke);
        }

        dstroke = dstrokeArray.join(" ");
        var SVG_NAMESPACE = "http://www.w3.org/2000/svg";
        var svg1 = document.createElementNS(SVG_NAMESPACE, "svg");
        var path1 = document.createElementNS(SVG_NAMESPACE, "path");
        path1.setAttribute("d",dstroke);
        svg1.innerHTML = path1;
        document.getElementById('pathlength').appendChild(svg1);
